# Comprehensive Warehouse Management System Demo
...
if ($runningServices.Count -eq 0) {
    Write-Host "`n⚠️  No services are running. Please start services first:" -ForegroundColor Yellow
    Write-Host "   .\start-all-services.ps1" -ForegroundColor White
    exit
}

Write-Host "`n🎯 DEMO SCENARIOS" -ForegroundColor Blue
Write-Host "=================" -ForegroundColor Blue

# Scenario 1: Inventory Management
if ($runningServices -contains "inventory-service") {
    Write-Host "`n📦 SCENARIO 1: Inventory Management" -ForegroundColor Yellow
    ...
    try {
        ...
        if ($inventory.Count -gt 0) {
            ...
            Write-Host "    ✅ Retrieved detailed information for item $($item.id)" -ForegroundColor Green
        }
    }
    catch {
        Write-Host "    ❌ Error testing inventory service: $($_.Exception.Message)" -ForegroundColor Red
    }
}

# Scenario 2: Order Management
if ($runningServices -contains "order-service") {
    Write-Host "`n🛒 SCENARIO 2: Order Management" -ForegroundColor Yellow
    Write-Host "-------------------------------" -ForegroundColor Yellow
    ...
    try {
        ...
        if ($orders.Count -gt 0) {
            ...
            Write-Host "    ✅ Retrieved detailed information for order $($order.orderNumber)" -ForegroundColor Green
            Write-Host "    📦 Order has $($orderDetail.orderItems.Count) items" -ForegroundColor Gray
        }
    }
    catch {
        Write-Host "    ❌ Error testing order service: $($_.Exception.Message)" -ForegroundColor Red
    }
}

# Scenario 3: Customer Management
if ($runningServices -contains "customer-service") {
    Write-Host "`n👥 SCENARIO 3: Customer Management" -ForegroundColor Yellow
    Write-Host "----------------------------------" -ForegroundColor Yellow
    ...
    try {
        ...
        Write-Host "    ❌ Error testing customer service: $($_.Exception.Message)" -ForegroundColor Red
    }
}
...
Write-Host "`n🔗 QUICK ACCESS LINKS" -ForegroundColor Blue
Write-Host "=====================" -ForegroundColor Blue
...
Write-Host "`n🎉 WAREHOUSE MANAGEMENT SYSTEM IS READY!" -ForegroundColor Green
